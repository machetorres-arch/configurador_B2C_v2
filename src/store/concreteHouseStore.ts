...
export const useConcreteHouseStore = create<ConcreteHouseState>((set, get) => ({
  ...
  updateOpening: (id, op) =>
    set((state) => ({
      openings: state.openings.map((item) => (item.id === id ? { ...item, ...op } : item)),
    })),
  ...
  loadPreset: (presetId) => {
    ...
    if (p) {
      set({
        ...
        slabType: p.slabType ?? (p.dimensions.roofType === 'dos_aguas_hormigon' ? 'dos_aguas_hormigon' : 'losa_hormigon_12cm'),
        openings: [...p.openings],
