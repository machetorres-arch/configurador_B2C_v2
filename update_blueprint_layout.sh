  ...
  const renderDrillingDetailsSVG = (part: Part, mod: ClosetModule, drawW: number, drawH: number, scale: number) => {
    ...
    if (isLateral || isBaseOrTecho) {
        ...
        if (!isBaseOrTecho) {
            elements.push(
                ...
                <text key="ft1" x={fixLeft/2} y={-10} fontSize="6" fill="#3b82f6" textAnchor="middle">50</text>
            );
        }
        ...
        if (!isBaseOrTecho) {
            elements.push(
                ...
                <text key="ft2" x={fixLeft/2} y={drawH + 16} fontSize="6" fill="#3b82f6" textAnchor="middle">50</text>
            );
        }
    }
    ...
    if (isLateral) {
        if (isDrawers) {
            ...
            elements.push(
                ...
                <text key="rt1" x={railX - 3} y={drawH / 2} fontSize="6" fill="#16a34a" transform={`rotate(-90 ${railX - 3} ${drawH / 2})`} textAnchor="middle">Eje Rieles: 37mm</text>
            );
        }
        ...
        if (mod.doors) {
            ...
            elements.push(
                ...
                <text key="ht1" x={hingeX + 3} y={topHoleY + 3} fontSize="6" fill="#ea580c">Base</text>,
                <text key="ht2" x={hingeX + 3} y={bottomHoleY + 3} fontSize="6" fill="#ea580c">Base</text>
            );
        }
    }
    ...
    if (isPuerta) {
        ...
        elements.push(
            ...
            <text key="pt3" x={-12} y={topHoleY/2} fontSize="6" fill="#ea580c" transform={`rotate(-90 -12 ${topHoleY/2})`} textAnchor="middle">100</text>,
            ...
            <text key="pt4" x={holeXLeft/2} y={topHoleY - 17} fontSize="5" fill="#ea580c" textAnchor="middle">22.5</text>
        );
    }
    ...
    if (isCajonStruct) {
        ...
        elements.push(
            ...
            <text key="ct1" x={edgeOffset + 2} y={drawH/2} fontSize="5" fill="#3b82f6" transform={`rotate(-90 ${edgeOffset + 2} ${drawH/2})`} textAnchor="middle">15mm</text>,
            <text key="ct2" x={drawW - edgeOffset + 2} y={drawH/2} fontSize="5" fill="#3b82f6" transform={`rotate(-90 ${drawW - edgeOffset + 2} ${drawH/2})`} textAnchor="middle">15mm</text>
        );
    }
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white text-black overflow-y-auto print-only-container">
      ...
      {printPages.map((page, pIdx) => {
        // Encontrar el tamaño máximo de la pieza en EL MÓDULO ENTERO para mantener consistencia de escala entre páginas
        ...
        // Escala conservadora (max 240px) para asegurar que 2 filas siempre quepan holgadas
        const scale = 240 / maxDimInModule;

        // Escala dinámica para vistas arquitectónicas (Garantiza que nunca sobrepasen el layout)
        const totalW = page.mod.width + state.depth + 40;
        const totalH = state.depth + state.height + 150;
        const viewScaleWidth = 350 / totalW;
        const viewScaleHeight = 600 / totalH;
        const viewScale = Math.min(viewScaleWidth, viewScaleHeight, 3.5);
              ...
              <div className="flex-1 p-8 grid grid-cols-12 gap-8 h-full pb-32">
                ...
                <div className="col-span-4 border-r-2 border-black/20 pr-6 flex flex-col items-center">
                   ...
                   <div className="flex flex-col items-center w-full mb-10">
                      <div className="text-sm font-bold mb-3">VISTA PLANTA</div>
                   ...
                   <div className="flex gap-10 w-full justify-center items-end">
                       <div className="flex flex-col items-center">
                          <div className="text-sm font-bold mb-3">VISTA FRONTAL</div>
                       ...
                       <div className="flex flex-col items-center">
                          <div className="text-sm font-bold mb-3">VISTA LATERAL</div>
                ...
                <div className="col-span-8 flex flex-col">
                    ...
                    <div className="grid grid-cols-4 gap-x-10 gap-y-24 overflow-visible pr-2 w-full">
                      {page.parts.map((part, pIdx) => {
                         const drawW = Math.max(part.width * scale, 10);
                         const drawH = Math.max(part.length * scale, 10);

                         return (
                          <div key={pIdx} className="flex flex-col items-center relative group pt-2">
                            <div className="text-[11px] text-black font-bold mb-1 uppercase tracking-wider text-center w-full truncate" title={part.name}>{part.name}</div>
                            <div className="text-[10px] bg-black text-white px-3 py-1 rounded-full font-bold mb-2">{part.qty} UN</div>
                            <div className="text-[9px] text-slate-700 mb-6">{part.material} {part.thickness}mm</div>
